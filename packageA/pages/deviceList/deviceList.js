...
Page({
  ...
  lockAotuOpen: function () {
    ...
    if (that.data.lockData) {
      wx.showModal({
        title: "提示",
        content:
          "请打开手机蓝牙，靠近门锁操作！锁常开=开锁后不会自动关锁，除非收到关锁指令。您确认设置锁常开吗？",
        complete: (res) => {
          if (res.confirm) {
            wx.showLoading({
              title: "请靠近门锁",
  ...
  lockAotuClose: function () {
    ...
    if (that.data.lockData) {
      wx.showModal({
        title: "提示",
        content:
          "请打开手机蓝牙，靠近门锁操作！锁自动关=每次开锁10秒后，锁会自动关闭。您确认设置锁自动关吗？",
        complete: (res) => {
          if (res.confirm) {
            wx.showLoading({
              title: "请靠近门锁",
  ...
  resetLock: function () {
    ...
    wx.showModal({
      title: "重要提示",
      content:
        "恢复出厂将还原锁未初始状态! 系统将不能控制,可再次通过初始化门锁重新恢复。",
      complete: (res) => {
        if (res.cancel) {
        }
        if (res.confirm) {
  ...
  initLock: function () {
    wx.showModal({
      title: "提示",
      content:
        "仅当门锁发出‘请添加管理员’时，门锁才可以被初始化！一般用于第一次安装锁使用，正常使用的门锁不需要初始化！",
      complete: (res) => {
        if (res.cancel) {
        }
        if (res.confirm) {
  ...
  configQrPrice(e) {
    ...
    var sn = e.currentTarget.dataset.sn;
    http.request(
      "/member/store/getQrConfig?sn="+sn,
      "1",
      "post",
      {
      },
      ...
      function success(info) {
        if (info.code == 0) {
          that.setData({
            qrSn: sn,
            showQrConfig: true,
            qrPrice: info.data.qrPrice,
            qrHour: info.data.qrHour,
          });
        } else {
          wx.showModal({
            content: "操作失败:" + info.msg,
  ...
  saveQrConfig() {
    var that = this;
    if (that.data.qrHour && that.data.qrPrice) {
      http.request(
        "/member/store/setQrConfig",
        "1",
        "post",
        {
          qrHour: that.data.qrHour,
          qrPrice: that.data.qrPrice,
          sn: that.data.qrSn,
        },
        app.globalData.userDatatoken.accessToken,
        "",
